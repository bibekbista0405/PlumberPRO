...
const developers = [
  { name: 'Anil Gupta', role: 'Co-Founder & Developer', bio: 'Anil works on PlumbPro\'s product and engineering, focused on making the booking and job-tracking experience clear for both customers and plumbers.' },
  { name: 'Bibek Bista', role: 'Co-Founder & Developer', bio: 'Bibek works on PlumbPro\'s platform and backend systems, focused on keeping the marketplace reliable — real availability, real verification, real data.' },
];

function About() {
  useSEO({
    title: 'About PlumbPro',
    description: 'PlumbPro is a location-aware plumbing service marketplace built in Nepal by two student developers, Anil Gupta and Bibek Bista.',
    path: '/about',
  });
  return <main className="about-page">
    ...
    <section className="about-devs">
      <div className="about-devs-head"><span className="eyebrow">THE TEAM</span><h2>Built by two students, end to end.</h2><p>PlumbPro is designed and built by two students as a real, working product — not a class exercise left unfinished.</p></div>
      <div className="about-devs-grid">
        {developers.map(dev => (
          <div className="dev-card" key={dev.name}>
            <div className="dev-avatar" aria-hidden="true">{dev.name.split(' ').map(n => n[0]).join('')}</div>
            <h3>{dev.name}</h3>
