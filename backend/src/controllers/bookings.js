...
SELECT b.*, s.name AS service_name, s.icon AS service_icon,
       ...
       p.name AS plumber_name, r.id AS review_id, r.rating AS review_rating
FROM bookings b
  ...
  try {
    ...
    if (plumber_id) {
      const [plumbers] = await connection.query(`SELECT u.id FROM users u JOIN plumber_profiles pp ON pp.user_id=u.id
        WHERE u.id=? AND u.role='plumber' AND u.status='active' AND pp.verified=1 AND pp.available=1`, [plumber_id]);
      if (!plumbers.length) return res.status(400).json({ message: 'Selected plumber is unavailable or not verified.' });
    }
    ...
    if (plumber_id) await createNotification(plumber_id, 'New booking request', `A customer requested a booking for ${booking_date} at ${booking_time}. Review it from your dashboard.`);
...
async function cancelBooking(req, res, next) {
  ...
  try {
    ...
    await connection.commit();
    if (rows[0].plumber_id) await createNotification(rows[0].plumber_id, 'Booking cancelled', `The customer cancelled booking #${req.params.id}.`);
    res.json({ message: 'Booking cancelled.' });
  } catch (err) { try { await connection.rollback(); } catch {} next(err); } finally { connection.release(); }
}

async function adminBookings(req, res, next) {
  try { const [rows] = await pool.query(bookingSelect + ' ORDER BY b.created_at DESC'); res.json({ bookings: rows }); } catch (err) { next(err); }
}
...
async function updateBooking(req, res, next) {
  ...
  try {
    ...
    // Admins get override authority so they can intervene when a plumber is unresponsive
    // (PRD section 16), but still cannot resurrect a booking that already reached a terminal state.
    ...
    await logHistory(connection, req.params.id, status, req.user.id, isReassignment ? (note || 'Reassigned by admin.') : note);
    await connection.commit();
    if (isReassignment) await createNotification(Number(plumber_id), 'You were assigned a booking', `An admin assigned you booking #${req.params.id}. Review it from your dashboard.`);
    await createNotification(current.customer_id, 'Booking updated', `An admin updated booking #${req.params.id} to "${status.replace('_', ' ')}".`);
...
async function plumberUpdateBooking(req, res, next) {
  ...
  try {
    ...
    // Re-check availability and double-booking at acceptance time (PRD section 15).
    if (status === 'confirmed') {
    ...
    if (statusMessages[status]) await createNotification(current.customer_id, 'Booking update', statusMessages[status]);
    const [updated] = await pool.query(bookingSelect + ' WHERE b.id = ?', [req.params.id]);
    res.json({ booking: updated[0] });
  } catch (err) { try { await connection.rollback(); } catch {} next(err); } finally { connection.release(); }
}

module.exports = { createBooking, myBookings, getBooking, cancelBooking, adminBookings, updateBooking, plumberBookings, plumberUpdateBooking, TRANSITIONS };
