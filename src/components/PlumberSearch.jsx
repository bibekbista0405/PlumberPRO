...
function PlumberSearch() {
  ...
  const performSearch = useCallback(async (nextCoords) => {
    ...
    try {
      ...
      if (!results.length) {
        setError(nextCoords
          ? 'No verified and available plumber was found within your selected area.'
          : 'No verified and available plumber matched that search. Try a nearby area or a wider radius.');
      }
    } catch (err) {
  ...
  const pickArea = (area) => {
    ...
    searchPlumbers({ q: area, service, radius }).then(d => {
      ...
      setPlumbers(results);
      if (!results.length) setError('No verified and available plumber matched that area yet.');
  ...
  const toggleService = (name) => {
    ...
    if (searched) {
      ...
      searchPlumbers({ q: query.trim(), service: next, lat: mode === 'location' ? coords?.lat : undefined, lng: mode === 'location' ? coords?.lng : undefined, radius })
        .then(d => { const results = d.plumbers || []; setPlumbers(results); setError(results.length ? '' : 'No verified and available plumber matched those filters.'); })
                ...
                <div className="plumber-card-top">
                  ...
                  <div className="plumber-card-heading">
                    <div className="plumber-name-row">
                      <h3>{p.name}</h3>
                      <span className="verified-badge"><Icon name="shieldCheck" size={12} /> Verified</span>
                    </div>
